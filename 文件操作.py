...
# encoding()   文件编码
# closed()      判断文件是否关闭
# seek(offset，whence) offset偏移量（正数向前，负数向后）
# whence 0开头 1现在 2末尾 控制文件光标，文件需要使用b方式打开。
# tell()          返回文件的光标位置
# truncate(size)     只有写文件用，size保留多少位，0位清空文件。
# with open("1.txt","w",encoding=ENCODING) as f:
#     f.write("a\nb\nc\n")
...
# a = "dockerroot:x:496:492:Docker User:/var/lib/docker:/sbin/nologin"
# a=a.split(":")
# print(a[2])
...
with codecs.open("passwd","r") as f:
    b=sorted(f.readlines(),key=lambda a:int(a.split(":")[2]))
